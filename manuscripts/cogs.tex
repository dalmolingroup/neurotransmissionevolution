...
\StringTok{  }\KeywordTok{filter}\NormalTok{(taxid }\OperatorTok{%in%}\StringTok{ }\NormalTok{string_eukaryotes[[}\StringTok{"taxid"}\NormalTok{]]) }\OperatorTok{%>%}
\StringTok{  }\CommentTok{# leave only proteins which are part of cogs of interest}
\StringTok{  }\KeywordTok{filter}\NormalTok{(cog_id }\OperatorTok{%in%}\StringTok{ }\NormalTok{cogs_of_interest[[}\StringTok{"cog_id"}\NormalTok{]])}
