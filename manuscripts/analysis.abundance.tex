...
\CommentTok{# tick_function <- function(skip_head = 1, skip_tail = 1) \{}
\CommentTok{#   function(x)\{}
\CommentTok{#     breaks <- seq(x[2], 0, length.out = 4) %>% head(-skip_head) %>% tail(-skip_tail)}
\CommentTok{#     ceiling(breaks/5)*5}
\CommentTok{#   \}}
\CommentTok{# \}}
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}
     ...
     \DataTypeTok{max_abundance =}\NormalTok{ abundance[lca }\OperatorTok{<=}\StringTok{ }\DecValTok{29}\NormalTok{] }\OperatorTok{%>%}\StringTok{ }\NormalTok{\{ }\KeywordTok{mean}\NormalTok{(.) }\OperatorTok{+}\StringTok{ }\DecValTok{3}\OperatorTok{*}\KeywordTok{sd}\NormalTok{(.) \}}
