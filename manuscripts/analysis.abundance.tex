...
\StringTok{  }\KeywordTok{count}\NormalTok{(taxid, cog_id,  }\DataTypeTok{name =} \StringTok{"cog_size"}\NormalTok{) }\OperatorTok{%>%}
...
Abundance is finally computed as the number of proteins pertaining to a
function divided by the number of neurotransmission COGs in a species.
...
\StringTok{  }\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{abundance =} \KeywordTok{mean}\NormalTok{(cog_size)) }\OperatorTok{%>%}
...
\CommentTok{# This tick function is used in scale_y_continuous to display only innermost ticks}
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}
     \CommentTok{# max_abundance = max(abundance[lca <= 29])}
     \DataTypeTok{max_abundance =}\NormalTok{ abundance[lca }\OperatorTok{<=}\StringTok{ }\DecValTok{29}\NormalTok{] }\OperatorTok{%>%}\StringTok{ }\NormalTok{\{ }\KeywordTok{mean}\NormalTok{(.) }\OperatorTok{+}\StringTok{ }\DecValTok{2}\OperatorTok{*}\KeywordTok{sd}\NormalTok{(.) \}}
\NormalTok{    ,}\DataTypeTok{abundance     =} \KeywordTok{ifelse}\NormalTok{(abundance }\OperatorTok{>=}\StringTok{ }\NormalTok{max_abundance, }\KeywordTok{pmin}\NormalTok{(max_abundance, }\DecValTok{100}\NormalTok{), }\KeywordTok{pmin}\NormalTok{(abundance, }\DecValTok{100}\NormalTok{))}
\NormalTok{  )}
...
\StringTok{  }\KeywordTok{geom_bar}\NormalTok{(}
     \KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ ncbi_name, }\DataTypeTok{y =}\NormalTok{ abundance, }\DataTypeTok{fill =}\NormalTok{ annotation, }\DataTypeTok{color =} \KeywordTok{after_scale}\NormalTok{(}\KeywordTok{darken}\NormalTok{(fill, }\FloatTok{0.1}\NormalTok{)))}
...
\StringTok{  }\KeywordTok{ylab}\NormalTok{(}\StringTok{"Average protein count in neural COGs"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\CommentTok{# Scales ----------------}
\StringTok{  }\KeywordTok{scale_y_continuous}\NormalTok{(}\DataTypeTok{breaks =} \KeywordTok{tick_function}\NormalTok{(), }\DataTypeTok{minor_breaks =} \OtherTok{NULL}\NormalTok{) }\OperatorTok{+}
...
\StringTok{  }\CommentTok{# coord_cartesian(ylim = c(0, 100)) +}
...
\NormalTok{abundance_plot }\OperatorTok{%+%}\StringTok{ }\NormalTok{abundance_by_function}
\end{Highlighting}
\end{Shaded}
...
\caption{Abundance values by species. Species are ordered like in Supplementary Figure S1.}\label{fig:unnamed-chunk-5}
\end{figure}
...
\StringTok{  }\KeywordTok{geom_bar}\NormalTok{(}
    \KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ clade_name, }\DataTypeTok{y =}\NormalTok{ abundance, }\DataTypeTok{fill =}\NormalTok{ annotation, }\DataTypeTok{color =} \KeywordTok{after_scale}\NormalTok{(}\KeywordTok{darken}\NormalTok{(fill, }\FloatTok{0.1}\NormalTok{)))}
...
Breaking the data even further into the number of proteins in each
neurotransmission COG.
...
\CommentTok{# Simply joining cog annotation and sizes}
...
\StringTok{  }\KeywordTok{scale_y_continuous}\NormalTok{(}\DataTypeTok{breaks =} \KeywordTok{tick_function}\NormalTok{(}\DecValTok{2}\NormalTok{, }\DecValTok{1}\NormalTok{), }\DataTypeTok{minor_breaks =} \OtherTok{NULL}\NormalTok{) }\OperatorTok{+}
