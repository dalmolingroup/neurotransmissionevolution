...
a <- string_eukaryotes %>%
  ...
  group_by(root) %>%
  mutate(lca_size = n_distinct(new_taxid)) %>%
  
  mutate(clade_name = strsplit(lineage_txt, "; ")) %>%
  unnest(clade_name) %>%
  
  group_by(new_taxid) %>%
  mutate(clade_depth = row_number()) %>%
  
  group_by(root, clade_name, clade_depth) %>%
  # filter(n() == lca_size) %>%
  summarise(clade_size = n()) %>%
  
  group_by(root) %>%
  summarise(collapse_at_depth = max(clade_depth[clade_size == max(clade_size)]))
  
  group_by(new_taxid) %>%
  filter(clade_depth == max(clade_depth))
...
roots_names <- string_eukaryotes %>%
  ...
  mutate(clade_name = coalesce(clade_name, lineage_list %>% flatten %>% map(length(lineage)+1) %>% unlist %>% unique %>% paste0(collapse="; "), clade_name)) #%>%
