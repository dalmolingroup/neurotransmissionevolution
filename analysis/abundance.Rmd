...
```
Plotting:
```{r  uncapped_abundances, fig.show="hide", fig.height=31.5, fig.width=7, fig.cap="Abundance values by species. Species are ordered like in Supplementary Figure 1."}
...
```{r fig.align="center", fig.height=4, fig.width=8, fig.cap="Species-specific average abundances, now averaged by clades."}
...
```{r all_abundances, fig.show="hide", fig.height=35, fig.width=15, fig.cap="Number of proteins in each neurotransmission COGs, for every species."}
...
ggplot(cog_abundance_collapsed) +
  ...
  theme(
    ...
    ,panel.grid.major.y = element_blank()
    ,strip.background   = element_blank()
    ,strip.text.x       = element_text(size = 6, angle = 90, hjust = 0.5, vjust = 0)
    ,strip.text.y       = element_text(size = 6, angle = 0, hjust = 0, vjust = 0.5)
    ,axis.text.x        = element_text(size = 4.5, angle = 90, vjust = 0, hjust = 0)
