...
ggplot() +
  geom_bar(data = ne_count %>% filter(annotation == "n"), aes(x = hybrid_root, y = n), fill = "#999999", stat = "identity") +
  geom_line(data = ne_count %>% filter(!annotation == "n"), aes(x = hybrid_root, y = n, group = annotation, color = annotation), size = 2) +
  scale_x_reverse(breaks = seq(0, 40, by = 1), expand = c(0,0), minor_breaks = NULL) +
  geom_vline(data = vl, aes(xintercept = xintercept), alpha = 0.25) +
  geom_text(data = vl, aes(x = xintercept, label = text, y=100), angle=90, hjust=0)
