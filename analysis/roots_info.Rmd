...
cog_roots <- read_tsv("../inst/extdata/cogs_roots.tsv")
```
...
# plot
```{r}
root_text <- root_extant %>% filter(root %in% ne_count[["root"]])

text <- geom_text(data = root_text, aes(x = root, label = branch, y=75), angle=90, hjust=0)
