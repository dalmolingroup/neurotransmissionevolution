...
### Manuscript figure 1
```{r fig1, fig.align="center", fig.height=17, fig.width=12, fig.cap="Unedited manuscript figure 1. The human neurotransmission network with nodes colored by neurotransmitter systems (top) and neurotransmission functions (bottom)."}
# Common elements
...
fig1b <- ggplot() +
  ...
  theme_facet +
  plot_text +
  scale_radius(range = c(4, 12), guide = FALSE) +
  guides(fill="legend")
...
```
```{r include=FALSE}
ggsave("plots/fig1_raw.pdf", width = 12, height = 17, useDingbats = F)
```
...
```
```{r include=FALSE}
ggsave("plots/fig3_raw.pdf", width = 12, height = 17, useDingbats = F)
```
