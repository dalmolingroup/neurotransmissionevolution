...
color_mappings <- c(
  ...
  ,"past"                  = "#FFFFFF"
  ,"other"                 = "#A0A0A0"
  ,"other_edge"            = rgb(0.7, 0.7, 0.7)
  ,"ne_border"             = "#000000"
)

node_fill_scale  <- scale_fill_manual(values = color_mappings)
node_color_scale <- scale_color_manual(values = color_mappings %>% darken(0.2))
...
### Manuscript figure 1
```{r fig1_raw, fig.show="hide", fig.align="center", fig.height=8, fig.width=6, fig.cap="Unedited manuscript figure 1. The human neurotransmission network with nodes colored by neurotransmitter systems (left) and neurotransmission functions (right)."}
####################
## Common elements
####################
...
![test.](figs/analysis.network.fig1_raw-1){height=90%}
...
```{r fig3_raw, fig.show="hide", fig.align="center", fig.height=8, fig.width=5.5, fig.cap="Unedited manuscript figure 3. The human neurotransmission network with nodes colored by neurotransmitter systems and neurotransmission functions.", dev.args = list(pdf = list(useDingbats = FALSE))}
# Finding which genes should be drawn
...
```{r fig4_raw, fig.show="hide", fig.align="center", fig.height=2, fig.width=10, fig.cap="Unedited manuscript figure 4. The human neurotransmission network with nodes rooted between roots 30 (Human-Choanoflagellata LCA) and 26 (Human-Cnidaria LCA)."}
...
![test.](figs/analysis.network.fig4_raw-1){width=100%}
...
```{r fig_sup_systems, fig.show="hide", fig.align="center", fig.height=20, fig.width=7.5, fig.cap="The human neurotransmission network with nodes rooted at each human LCA."}
system_plots   <- list()
function_plots <- list()
...
iwalk(roots, ~ {
  ...
  plot_text <- geom_text(data = current_genes, text_aes, size = 0.75, nudge_y = 4, alpha = 0.5)
...
# Printing and saving
arrangeGrob(grobs = system_plots, ncol = 2) %>% grid::grid.draw()
```
\clearpage
...
```{r fig_sup_functions, fig.show="hide", fig.align="center", fig.height=20, fig.width=7.5, fig.cap="The human neurotransmission network with nodes rooted at each human LCA."}
arrangeGrob(grobs = function_plots, ncol = 2) %>% grid::grid.draw()
```
...
ggplot() +
  ...
  facet_grid(-facet_root ~ system, switch = "y") +
  theme_void() +
  theme(
     strip.text.y     = element_text(angle = 90)
    ,strip.background = element_rect(color = "#c0c0c0", fill = "#c0c0c0", size = 0.5, linetype = "solid")
    ,panel.border     = element_rect(color = "#c0c0c0", fill = NA, size = 0.5, linetype = "solid")
  ) +
