...
vertices %<>% mutate(
  ...
  color_node = ifelse(ne, "#000000", element_colors[annotation] %>% darken(0.25)),
...
```{r}
#finding which genes should be drawn
current_genes <- vertices %>% filter(hybrid_root == 37)
...
## SAVING
############
base <- ggplot() + plot_edges + plot_scales
...
```{r}
#finding which genes should be drawn
current_genes <- vertices %>% filter(hybrid_root < 37 & hybrid_root >= 26)
past_genes <- vertices %>% filter(hybrid_root == 37)
...
## SAVING
############
base <- ggplot() + plot_edges + plot_past + plot_scales
...
upset(
  vertices %>% filter(hybrid_root == 37) %>% select(sistemas)
  ,mb.ratio = c(0.7, 0.3)
  ,order.by = "freq"
...
upset(
  vertices %>% filter(hybrid_root < 37 & hybrid_root >= 26) %>% select(sistemas)
  ,mb.ratio = c(0.7, 0.3)
  ,order.by = "freq"
  ,mainbar.y.label = "System Intersections"
  ,sets.x.label = "Genes per system"
