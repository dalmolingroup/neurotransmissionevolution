...
```{r}
# Recreating the vertices data.frame, now with layout coordinates (lazy)
vertices <- igraph::as_data_frame(g, what = "vertices") %>% rename(string_id = name)

# The edges data.frame will be used to draw lines with geom_line
...
### Figure 1
```{r fig.align="center", fig.height=6, fig.width=14, fig.cap="The human neurotransmission network with nodes colored by neurotransmitter systems (left) and neurotransmission functions (right)."}
####################
## Common elements
####################
...
### Figure 3  
The process for generating Figures 3 and 4 (as well as Supplementary Figures 2 and 3) is roughly the same. It consists of finding what nodes have numeric roots in a given range. In our analysis, the largest root is numbered 37 and represents the furthest human common ancestor in the cladogram (the Human-Metamonada LCA, as seen in previous sections). Root #1 is represented by _Homo sapiens_ itself.  
...
```{r fig.align="center", fig.height=5.5, fig.width=14, fig.cap="The human neurotransmission network with nodes colored by neurotransmitter systems and neurotransmission functions."}
# Finding which genes should be drawn
...
```{r fig.align="center", fig.height=2, fig.width=10, fig.cap="The human neurotransmission network with nodes rooted between roots 30 (Human-Choanoflagellata LCA) and 26 (Human-Cnidaria LCA)."}
...
Supplementary figures 2 and 3 help us see what nodes have been rooted at each LCA. Nodes rooted at previous LCAs are painted white.
```{r}
system_plots   <- list()
function_plots <- list()
