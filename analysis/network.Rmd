...
cog_roots <- read_tsv("geneplast_roots_modified.tsv", col_types = "ci")

roots_extant_clades <- read_tsv("geneplast_clade_names_modified.tsv", col_types = "ic")
```
...
roots_titles <- vertices %>% arrange(-root) %>% pull(clade_name) %>% unique %>% paste0(" LCA (",roots,")")
...
fig4 <- map2(roots[idx], roots_titles[idx], ~ {
  ...
  #labels
  plot_text <- geom_text(data = current_genes, text_aes, size = 0.8, vjust = -0.5, nudge_y = 1, alpha=0.5)
...
ggsave(file = "plots/fig4_2.pdf", arrangeGrob(grobs = fig4, ncol = 5), width = 9*0.8, height = 5*0.8, onefile=F, useDingbats=FALSE)
```
